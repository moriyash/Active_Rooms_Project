...
const CampusMapView = () => {
  ...
  const loadCampusData = async () => {
    try {
      ...
      // Add sensor count and floor count to each building
      const enhancedBuildings = await Promise.all(
        buildingsData.map(async (building) => {
          try {
            const floors = await ApiService.getFloorsByBuilding(building.id);
            const allSensors = await ApiService.getAllSensors();
            const buildingSensors = allSensors.filter(sensor => 
              floors.some(floor => floor.id === sensor.area_id)
            );
            
            return {
              ...building,
              floorCount: floors.length,
              sensorCount: buildingSensors.length,
            ...
            return {
              ...
              floorCount: 0,
              sensorCount: 0,
              mapX: building.mapX || 50,
              mapY: building.mapY || 50
            };
          }
        })
      );
  ...
  const getBuildingStatus = (building) => {
    // Get all sensors for this building's floors
    const buildingSensors = Object.values(sensorsStatus).filter(sensor => {
      return building.id === sensor.areaId; // Direct match if sensor is in building
    });
  ...
  const getOverallStats = () => {
    const totalSensors = Object.keys(sensorsStatus).length;
    const activeSensors = Object.values(sensorsStatus).filter(s => s.status === 'active').length;
    const alertSensors = Object.values(sensorsStatus).filter(s => s.alert).length;
