...
const CampusMapView = () => {
  ...
  const loadCampusData = async () => {
    try {
      ...
      // Try to get buildings from server
      const buildingsData = await ApiService.getAllBuildings();
      console.log('Buildings data received:', buildingsData);
      
      // Even if there are some errors with floors/sensors, if we have buildings, show them
      if (buildingsData && buildingsData.length > 0) {
        setBuildings(buildingsData);
        setError(null); // Clear any previous errors
      } else {
        // Only show error if we truly have no buildings
        setError(null); // Don't show error for empty data
        setBuildings([]); // Set empty array
      }
    } catch (err) {
      console.error('Error loading campus data:', err);
      // Only show error if it's a real server/connection error
      if (err.message.includes('fetch') || err.message.includes('network') || err.message.includes('Failed to')) {
        ...
        setBuildings([]); // Set empty buildings
      }
    } finally {
      setLoading(false);
    }
  ...
  // Show message if no buildings found
        ...
        <div className="main-content">
          <div className="buildings-section">
            <div className="section-header">
              <h1>אין בניינים זמינים</h1>
              <p>לא נמצאו בניינים במערכת. אנא פנה למנהל המערכת.</p>
            </div>
            <button onClick={loadCampusData} className="retry-button">
              רענן נתונים
            </button>
        ...
        {!selectedBuilding ? (
                ...
                >
                  ...
                  <div className="building-info">
                    ...
                    <div className="building-stats">
                      <span>קומות: {building.floors ? building.floors.length : 0}</span>
                      <span>חדרים: {building.floors ? building.floors.reduce((total, floor) => total + (floor.sensors ? floor.sensors.length : 0), 0) : 0}</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="floor-section">
            ...
            {selectedBuilding.floors && selectedBuilding.floors.length > 0 ? (
                ...
                {selectedFloor && (
                  <div className="floor-map-container">
                    ...
                    <div className="floor-stats">
                      ...
                      <div className="stat-card">
                        <span className="stat-number">{selectedFloor.sensors ? selectedFloor.sensors.reduce((total, s) => total + (s.maxCapacity || 0), 0) : 0}</span>
              ...
              <div className="section-header">
                <h2>לא נמצאו קומות לבניין זה</h2>
                <p>אנא פנה למנהל המערכת להוספת קומות.</p>
              </div>
            )}
          </div>
        )}
